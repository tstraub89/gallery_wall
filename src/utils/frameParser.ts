export interface ParsedFrame {
    ...
    matted: { width: number; height: number } | null;
}
 ...
 * @returns {number|null}
 */
const parseDimensionValue = (dimStr: string | undefined): number | null => {
 ...
 * 
 * @param {string} line 
 * @returns {ParsedFrame|null} { width, height, mattedTo: { width, height } | null, originalString }
 */
export const parseFrameLine = (line: string): ParsedFrame | null => {
    ...
    const extractDims = (str: string) => {
        ...
        if (parts.length === 2) {
            ...
            const clean = (s: string) => s.replace(/[^0-9\s./]/g, '').trim();
...
export const parseFrameFile = (fileContent: string) => {
